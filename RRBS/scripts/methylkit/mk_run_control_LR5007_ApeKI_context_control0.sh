...
for i in $(seq 1 12); do
	...
	mkdir control_LR5007_ApeKI_control0_$1
    	cd control_LR5007_ApeKI_control0_$1
    	for f in ../*.$1.*bed; do echo $f ; ln -s $f ; done
	cp ../../MK_control_vs_LR5007_ApeKI_control0.Rmd . 
	echo "#!/bin/bash" > "MK_control_LR5007_ApeKI_"$chr".sh"
    	echo "module load R/x86_64/2.15.1" >> "MK_control_LR5007_ApeKI_"$chr".sh"
	echo "Rscript --no-save --no-restore --no-init-file --no-site-file -e \"library(knitr);chromosome <- '$chr';context<-'$1';knit2html('MK_control_vs_LR5007_ApeKI_control0.Rmd')\"" >> "MK_control_LR5007_ApeKI_"$chr".sh"
